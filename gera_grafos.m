...
for i = 1:2 %estados mentais
    for j = 1:7 %bandas de frequencias
        ...
        MeanC_cat{i,j} = cat(3, conn_matrix{:,i,j}); 
...
for i = 1:7 %Para cada frequencia
    for j = 1:30 %Para todos os pares de um determinado canal
%            p_teste{i,j} = ranksum(MeanC{1,i}(j,:),MeanC{2,i}(j,:),'tail','right'); % Fadigado - Normal
          ...
          % alcancada
          [ p_val{i,j},hip{i,j}] = ranksum(MeanC{1,i}(j,:),MeanC{2,i}(j,:),'tail','left'); % Fadigado - Normal
          [ p_val_fn{i,j},hip_fn{i,j}] = ranksum(MeanC{1,i}(j,:),MeanC{2,i}(j,:),'tail','right'); % Normal - Fadigado
    end
end
...
hip_mat = cell2mat(hip);
hip_mat_fn = cell2mat(hip_fn);

% Verifying channel with hip 
verif_corr_ocurr_fat = and(hip_mat,a_semsoma_logical')'; %30 x 7
verif_corr_ocurr_nor = and(hip_mat,b_semsoma_logical')'; %30 x 7
...
verif_corr_ocurr_and = and(verif_corr_ocurr_fat,verif_corr_ocurr_nor);
