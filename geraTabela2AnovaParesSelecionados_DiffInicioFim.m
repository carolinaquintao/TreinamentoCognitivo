...
for k=1:length(freqbdw_label)
    arquivo = ['tabelaFinal2_Diff_IniFim_' freqbdw_label{k} '_' 'tail' '_' 'left''.mat']
    for i=pares_selec
...
%         p1(i,2) = ranksum(heatmap_Antes_5f(i,1:4)',heatmap_Depois_5f(i,1:4)','tail','left');
        p1(i,1) = ranksum(heatmap_Antes_5i(i,str2num(freqbdw{k}))',heatmap_Antes_5f(i,str2num(freqbdw{k}))','tail','left');%right x>y  left x<y
        p1(i,2) = ranksum(heatmap_Depois_5i(i,str2num(freqbdw{k}))',heatmap_Depois_5f(i,str2num(freqbdw{k}))','tail','left');
