function [SW,SWE,SWW, CC, CCrnd, L,Lrnd] = allSmallWorldness( A, numRndNet, w )
% function [SW, SWE, SWZ, SWZl, SWW] = allSmallWorldness( AM, numRndNet, w )
%ALLSMALLWORLDNESS Summary of this function goes here
...
% function [SW, SWE, SWZ, SWZl, SWW] = AllSmallWorldness( AM, numRndNet, w )
    ...
    % Calculation of the CC %Clusters coefficients
    CC = functionClusteringCoefficient( A );
    CCrnd = zeros( numRndNet, 1 );
    for k = 1 : numRndNet
        CCrnd(k) = functionClusteringCoefficient( rndNet{k} );
    end

    % Calculation of L and all distances %path length 
    [L, dist] = functionL( A );
