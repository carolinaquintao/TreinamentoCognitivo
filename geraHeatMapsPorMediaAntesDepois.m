...
mean_subject_5f = meanCell(hemisf_heatmap_vector_5f');
mean_subject_5i = meanCell(hemisf_heatmap_vector_5i');

max_value_all= max(max([mean_subject_5i{1},mean_subject_5f{1},mean_subject_5i{2},mean_subject_5f{2}]));
min_value_all = min(min([mean_subject_5i{1},mean_subject_5f{1},mean_subject_5i{2},mean_subject_5f{2}]));
...
j=2;
tempo = 'f'
nomeArquivo = [path_saving,'\vector_5' tempo '_meanSuj_periodo_',per{j}]

pc = pcolor(mean_subject_5f{j,1}');
...
%Mean Min
title(['vector_5' tempo '_periodo: ',per{j}],'Interpreter','none');
savefig(p1,[nomeArquivo,'.fig']);
saveas(p1,[nomeArquivo,'.png']);
close;
