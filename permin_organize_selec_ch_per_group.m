...
if exist('./S11D/Connectivity_S11D/RT_OA_raw/conn_matrix_rt_permin.mat') == 2 %Se existir
    load('./S11D/Connectivity_S11D/RT_OA_raw/conn_matrix_rt_permin.mat');
else
    load('./S11D/Connectivity_S11D/RT_OA_raw/conn_matrix_freq_permin.mat');
    %Taking mean in the frequency
    conn_matrix_rt_permin = cellfun(@(x) mean(x,3),conn_matrix_freq_permin,'un',0);
    save('./S11D/Connectivity_S11D/RT_OA_raw/conn_matrix_rt_permin.mat','conn_matrix_rt_permin')
end
...
if exist('./S11D/Connectivity_S11D/PosBench_raw/conn_matrix_rtPos_permin.mat') == 2
    load('./S11D/Connectivity_S11D/PosBench_raw/conn_matrix_rtPos_permin.mat');
else   
    load('./S11D/Connectivity_S11D/PosBench_raw/conn_matrix_freq_permin.mat');
    conn_matrix_rtPos_permin = cellfun(@(x) mean(x,3),conn_matrix_freq_permin,'un',0);

    save('./S11D/Connectivity_S11D/PosBench_raw/conn_matrix_rtPos_permin.mat','conn_matrix_rtPos_permin')

end
% Benchmarck

%Valores sem tirar a media no tempo 
if exist('./S11D/Connectivity_S11D/Benchmarck/conn_matrix_3i_permin.mat') == 2
    load('./S11D/Connectivity_S11D/Benchmarck/conn_matrix_3i_permin.mat');
    load('./S11D/Connectivity_S11D/Benchmarck/conn_matrix_3f_permin.mat');
else
    
    load('./S11D/Connectivity_S11D/Benchmarck/conn_matrix_freq_permin.mat');
    conn_matrix_3i_permin = conn_matrix_freq_permin(:,:,:,1:3);
    conn_matrix_3f_permin = conn_matrix_freq_permin(:,:,:,4:6);

    save('./S11D/Connectivity_S11D/Benchmarck/conn_matrix_3i_permin.mat');%25x1x7x3
    save('./S11D/Connectivity_S11D/Benchmarck/conn_matrix_3f_permin.mat');
end
    
    conn_matrix_bench3i_permin = cellfun(@(x) mean(x,3),conn_matrix_3i_permin,'un',0);
    conn_matrix_bench3f_permin = cellfun(@(x) mean(x,3),conn_matrix_3f_permin,'un',0);

    save('./S11D/Connectivity_S11D/Benchmarck/conn_matrix_bench3i.mat','conn_matrix_bench3i_permin');
    save('./S11D/Connectivity_S11D/Benchmarck/conn_matrix_bench3f.mat','conn_matrix_bench3f_permin');
...
for i = 1:3
    conn_matrix_rt_groups{i} = conn_matrix_rt_permin(v_grupos_idx{i},:,:,:);
    conn_matrix_rtPos_groups{i} = conn_matrix_rtPos_permin(v_grupos_idx{i},:,:,:);
    conn_matrix_bench3i_groups{i} = conn_matrix_bench3i_permin(v_grupos_idx{i},:,:,:);
    conn_matrix_bench3f_groups{i} = conn_matrix_bench3f_permin(v_grupos_idx{i},:,:,:);
end
    ...
    for k = 1:size(conn_matrix_rt_groups{i},1) %Quantidade de sujeitos por grupo
        for l = 1:length(ch_selec_pos_rt_artificial) %Pares selecionados
            for j =1:19 %All pairs combinations
                for t = 1:3 %para cada um dos 3 min 
%           varia_conn(w) = a_nor(w)/a_fat(w);
%           varia_conn(w) = MeanC{2,4}(16,w)/MeanC{1,4}(16,w);
                    varia_conn_rtPos_rt_selec_permin{i,t}{k,l,j} = conn_matrix_rtPos_groups{i}{k,1,v_ch_selec_rt_pos{l}(2),t}(v_ch_selec_rt_pos{l}(1),j) ./ conn_matrix_rt_groups{i}{k,1,v_ch_selec_rt_pos{l}(2),t}(v_ch_selec_rt_pos{l}(1),j);
                    varia_conn_bench3f_bench3i_selec_permin{i,t}{k,l,j} = conn_matrix_bench3f_groups{i}{k,1,v_ch_selec_bench_pos{l}(2),t}(v_ch_selec_bench_pos{l}(1),j) ./ conn_matrix_bench3i_groups{i}{k,1,v_ch_selec_bench_pos{l}(2),t}(v_ch_selec_bench_pos{l}(1),j);
...
save('./S11D/Ocorrencia/varia_conn_rtPos_rt_selec_permin.mat','varia_conn_rtPos_rt_selec_permin');
save('./S11D/Ocorrencia/varia_conn_bench3f_bench3i_selec_permin.mat','varia_conn_bench3f_bench3i_selec_permin');

%% Media dos sujeitos por grupo de desempenho

for i = 1:3 %Para cada grupo 
    for t = 1:3 %para cada minuto 
        varia_conn_rt_selec_meanSubj_permin{i,t} = permute(mean(cell2mat(varia_conn_rtPos_rt_selec_permin{i,t}),1),[2,3,1]);
        varia_conn_bench_selec_meanSubj_permin{i,t} = permute(mean(cell2mat(varia_conn_bench3f_bench3i_selec_permin{i,t}),1),[2,3,1]);
    end
end
